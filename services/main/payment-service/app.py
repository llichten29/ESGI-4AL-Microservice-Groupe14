...
def create_app():
    ...
    register_openapi_validation(app, service_name="payment-service")
    ...
    service = PaymentService(repository=repo, broker=broker)
...
if __name__ == '__main__':
    app = create_app()
    app.run(host='0.0.0.0', port=app.config.get('SERVICE_PORT', 8004), debug=app.config.get('DEBUG', False))
